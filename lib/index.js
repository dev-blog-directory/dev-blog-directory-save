...
function loadBlogs() {
  ...
  return Promise.resolve()
    ...
    .then((blogs) =>
      blogs.filter((blog) => blog && typeof blog.url === 'string')
    )
    .then((blogs) => {
      blogs.forEach((blog) => {
...
function validateCategory(tag) {
  return /^[A-Za-z\d][A-Za-z\d-]*[A-Za-z\d]$/.test(tag);
}

function validateTag(tag) {
  const validTags = ['c++', 'c/c++', 'c#', '.net'];
  return /^[a-z\d][a-z\d-]*[a-z\d]$/.test(tag) || validTags.includes(tag);
}
  ...
  if (doc.langs) {
    ...
    doc.langs = doc.langs.map((lang) => {
      if (!ISO6391.validate(lang) && !LocaleCode.validate(lang)) {
        throw new TypeError(
          'invalid language code: ' + lang + stringify(doc, 'langs')
        );
      }
      ...
      .map((tag) => {
        ...
        if (!validateCategory(tag)) {
          throw new TypeError(
            'invalid category: ' +
              orgTag +
              "\nonly alphabet, number, - are allowed. - can't appear at the start and the end." +
              stringify(doc, 'categories')
          );
        }
...
function _merge(doc) {
  ...
  return lockfile
    ...
    .then((array) => {
      const content = array.map((x) => dump(x)).join('');
...
function saveAll(docs, options) {
  ...
  return Promise.resolve()
    ...
    .then(() => {
      ...
      if (options.merge) {
        return Promise.all(docs.map((x) => _merge(x)));
      }

      return Promise.all(docs.map((x) => _save(x)));
    });
}
